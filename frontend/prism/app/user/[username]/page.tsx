...
const ContentItem: React.FC<{ title: string; value: string | null | undefined, link?: string }> = ({
  title,
  value,
  link,
}) => {
...
const OwnProfileInformation: React.FC = () => {
    return (
      <div className="mb-8">
        <div className="space-y-4">
          <ContentItem title="Email" value="user@example.com" />
          <ContentItem title="Bio" value="A short bio about the user." />
        </div>
        <Link href="/settings/profile" className="vaporwave-button mt-6 inline-block">Edit Profile</Link>
      </div>
    );
  };
  
  const OtherUserProfileInformation: React.FC<{ isLoggedIn: boolean }> = ({ isLoggedIn }) => {
    return (
      <div>
        {isLoggedIn && (
        ...
        <div className="space-y-4">
          <ContentItem title="Full Name" value="Other User Name" />
          <ContentItem title="Bio" value="Another short bio." />
        </div>
      </div>
    );
  };
  ...
  const UserProfileCard: React.FC<UserProfileProps> = ({
    isOwnProfile,
    profileData, // Still needed for ProfilePicture and ProfileHeader
    isLoggedIn,
  }) => {
    return (
      <div className="w-full max-w-3xl bg-gradient-to-br from-indigo-900 via-purple-900 to-pink-700 rounded-2xl p-8 shadow-2xl z-50 flex flex-col items-center">
        ...
        <div className="w-full">
          {isOwnProfile ? (
            ...
            <OtherUserProfileInformation isLoggedIn={isLoggedIn} />
          )}
        </div>
      </div>
    );
...
export default function UserProfilePage() {
  ...
  if (isLoading || !profileData) {
    return (
    <div className="flex flex-col items-center justify-center p-8 min-h-screen bg-vapor-dark bg-grid bg-size-grid">
      <div className="vaporwave-button animate-glow">
            Loading...
      </div>
    </div>
    );
